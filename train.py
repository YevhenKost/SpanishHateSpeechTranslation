...
def train_folds(args: argparse.Namespace) -> Dict[str, float]:
    ...
    train_f=data_loaded_dict["train_df"]
    eval_f=data_loaded_dict["eval_df"]
...
# Train the model
model.train_model(train_f, eval_data=eval_f)

# Optional: Evaluate the model. We'll test it properly anyway.
results = model.eval_model(eval_f, verbose=True)
